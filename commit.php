...
case 'edit':
    ...
    case 'people':
        $error = array();
        $movie_name = isset($_POST['people_fullname']) ?
            trim($_POST['people_fullname']) : '';
        if (empty($movie_name)) {
            $error[] = urlencode('Please enter a name.');
        }
        $movie_type = isset($_POST['people_isactor']) ?
            trim($_POST['people_isactor']) : '';
        if (empty($movie_type)) {
            $error[] = urlencode('Please select if is actor.');
        }
        $movie_year = isset($_POST['people_isdirector']) ?
            trim($_POST['people_isdirector']) : '';
        if (empty($movie_year)) {
            $error[] = urlencode('Please select if is director.');
        }
        if (empty($error)) {
            $query = 'INSERT INTO
                people
          ...
          header('Location:movie.php?action=edit&id=' . $_POST['people_id'] .
              '&error=' . join($error, urlencode('<br/>')));
        }
        if (empty($error)) {
            $query = 'UPDATE
